...
$import_file = $modx->getOption('import_file', $scriptProperties, '');
$import_filepath = $modx->getOption('base_path') . $import_file;
$export_file = $modx->getOption('export_file', $scriptProperties, '');
$export_filepath = $modx->getOption('base_path') . $export_file;
$settings = $modx->getOption('settings', $scriptProperties, '');

$export_action = $modx->getOption('export_action', $scriptProperties, 'download');

$joins = isset($scriptProperties['export_joins']) && !empty($scriptProperties['export_joins']) ? $modx->fromJson($scriptProperties['export_joins']) : false;
...
$save_import_settings = in_array('save_imp_settings', $settings_array) ? true : false;
$use_joins = ($joins && in_array('use_joins', $settings_array)) ? true : false;
...
if (!isset($scriptProperties['download']) || !($scriptProperties['download'])) {
    ...
    */
    ...
    if ($save_import_settings) {
        $settings = json_decode($settings_output, true);
        $import_settings = array();
        if (is_array($settings)) {
            foreach ($settings as $setting => $value) {
                if (strstr($setting, 'import_')) {
                    $import_settings[str_replace('import_', '', $setting)] = $value;
                }
            }
        }
        ...
        $cacheManager->writeFile($defaultSettingName, json_encode($import_settings));
    }
    ...
    switch ($export_action) {
        case 'download':
            ...
            return $modx->error->success('', array('download_filename' => basename($cacheName)));
            break;
        case 'local':
            $cacheManager = $modx->getCacheManager();
            $cacheManager->writeFile($export_filepath, $output);
            return $modx->error->success(basename($cacheName));
            break;
        case 'for_import':
            $cacheManager = $modx->getCacheManager();
            $cacheManager->writeFile($import_filepath, $output);
            return $modx->error->success(basename($cacheName));
            break;
    }
